...
const LatestNews = () => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <h2 className="text-xl font-bold text-gray-900 mb-4">Latest News & Analysis</h2>
      <div className="space-y-4">
        {newsItems.map((item, index) => (
          <div key={index} className={`border-l-4 p-4 rounded-r-lg ${sentimentColor[item.sentiment]}`}>
            <div className="flex items-start justify-between mb-2">
              <h3 className="font-semibold text-gray-900 text-sm leading-tight flex-1">{item.title}</h3>
              <ExternalLink className="w-4 h-4 text-gray-400 ml-2 flex-shrink-0 cursor-pointer hover:text-gray-600" />
            </div>
