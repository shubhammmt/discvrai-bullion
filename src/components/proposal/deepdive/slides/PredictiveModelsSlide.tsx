...
export const PredictiveModelsSlide: React.FC<Props> = ({ slide, slideNumber, totalSlides }) => {
  ...
  return (
    <DeepDiveSlideLayout slideNumber={slideNumber} totalSlides={totalSlides} sectionLabel="Personalization" sectionColor="bg-purple-600">
      ...
      {/* User profile bar */}
      <div className="bg-slate-100 border border-slate-200 rounded-lg px-3 py-1.5 mb-2 flex gap-4 text-[9px] text-slate-600">
        ...
        {features.map((f: any, i: number) => (
          <div key={i} className="bg-slate-50 border border-slate-200 rounded-lg p-2">
            <div className="flex justify-between items-start mb-1">
              <p className="text-[9px] font-bold text-slate-700 leading-tight">{f.name}</p>
              <span className={`text-[7px] font-bold px-1 py-0.5 rounded ${riskColors[f.riskLevel]}`}>{f.riskLevel}</span>
            </div>
            <p className="text-[8px] text-slate-500 mb-1">Weight: {f.weight} | Score: {f.score}</p>
            {f.details.map((d: string, j: number) => (
              <p key={j} className="text-[7px] text-slate-500 leading-tight">• {d}</p>
            ))}
          </div>
        ))}
      </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 mb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-red-700">Churn Probability: {churnProbability}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-slate-500">Score: {churnScore}/100</span>
            <div className="w-24 bg-slate-200 rounded-full h-2">
              <div className="bg-red-500 h-2 rounded-full" style={{ width: `${100 - churnScore}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-2">
          <p className="text-[10px] font-bold text-blue-700 mb-1"><Shield className="w-3 h-3 inline mr-1" />Proactive Interventions</p>
          {interventions.map((int: string, i: number) => (
            <p key={i} className="text-[8px] text-slate-600 leading-tight">• {int}</p>
          ))}
        </div>
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-2">
          <p className="text-[10px] font-bold text-emerald-700 mb-1">Expected Outcome</p>
