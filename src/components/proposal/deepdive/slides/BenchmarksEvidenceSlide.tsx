      ...
      <div className="h-full flex flex-col">
        ...
        <div className="grid grid-cols-3 gap-3 flex-1 min-h-0">
          {benchmarks.map((b: any, i: number) => (
            <div key={i} className={`${colors[i].bg} ${colors[i].border} border rounded-lg p-3 flex flex-col`}>
              <div className="flex items-center gap-2 mb-2">
                <div className={`${colors[i].accent} text-white p-1.5 rounded-md`}>
                  {icons[i]}
                </div>
                <h3 className={`text-[11px] font-bold ${colors[i].text}`}>{b.title}</h3>
              </div>

              <div className="space-y-2 flex-1">
                {b.evidence.map((ev: any, j: number) => (
                  <div key={j}>
                    <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wide mb-0.5">{ev.label}</p>
                    <p className="text-[10px] text-slate-700 leading-snug">{ev.detail}</p>
                  </div>
                ))}
              </div>

              {b.keyMetric && (
                <div className={`mt-2 ${colors[i].accent} rounded-md px-3 py-1.5`}>
                  <p className="text-[9px] text-white/80 font-medium">{b.keyMetric.label}</p>
                  <p className="text-sm font-bold text-white">{b.keyMetric.value}</p>
                </div>
              )}

              {b.references && b.references.length > 0 && (
                <div className="mt-2 pt-1.5 border-t border-slate-200/60">
                  <p className="text-[8px] text-slate-400 font-medium mb-0.5">References</p>
                  {b.references.map((ref: string, k: number) => (
                    <div key={k} className="flex items-center gap-1">
                      <ExternalLink className="w-2 h-2 text-slate-400 shrink-0" />
                      <p className="text-[8px] text-slate-400 truncate">{ref}</p>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="mt-2.5 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 flex items-start gap-2">
