  ...
  return (
    <DeepDiveSlideLayout slideNumber={slideNumber} totalSlides={totalSlides} sectionLabel="AI Nudges" sectionColor="bg-blue-600">
      <div className="mb-2">
        ...
        <p className="text-base text-slate-500">{slide.subtitle} — <span className="font-semibold text-slate-700">{slide.content.example}</span></p>
      </div>
          ...
          return (
            <div key={i} className={`${colors.bg} border ${colors.border} rounded-lg p-4`}>
              <div className="flex items-center gap-2 mb-2">
                <span className={`${colors.badge} text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center`}>{step.step}</span>
              ...
              {step.message && (
                <div className="bg-white rounded p-2.5 mt-2 border border-amber-200">
                  <p className="text-[11px] text-slate-700 italic leading-snug">{step.message}</p>
                  <div className="flex gap-2 mt-2">
                    <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-medium">{step.channel}</span>
                    <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-medium">{step.timing}</span>
                  </div>
                  <div className="flex gap-2 mt-1.5">
                    {step.cta.map((c: string, k: number) => (
                      <span key={k} className="text-[10px] bg-blue-600 text-white px-2.5 py-0.5 rounded-full font-medium">{c}</span>
                    ))}
                  </div>
                </div>
              )}
              {step.metrics && (
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {step.metrics.map((m: any, j: number) => (
                    <div key={j} className="bg-white rounded px-2.5 py-1.5 border border-emerald-200">
                      <p className="text-[10px] text-slate-500">{m.label}</p>
                      <p className="text-xs font-bold text-emerald-700">{m.value}</p>
                    </div>
                  ))}
                </div>
              )}
