    ...
    <DeepDiveSlideLayout slideNumber={slideNumber} totalSlides={totalSlides} sectionLabel="Distributor" sectionColor="bg-teal-600">
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
          <p className="text-xs font-bold text-red-700 uppercase tracking-wider mb-2">❌ Traditional ({traditional.time})</p>
          <p className="text-[11px] text-slate-500 mb-1.5">Quality: {traditional.quality}</p>
          {traditional.steps.map((s: string, i: number) => (
            <p key={i} className="text-[11px] text-slate-600 leading-snug">{i + 1}. {s}</p>
          ))}
        </div>

        {/* AI-Enabled */}
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
          <p className="text-xs font-bold text-emerald-700 uppercase tracking-wider mb-2">✅ AI-Enabled ({aiEnabled.time})</p>
          <div className="bg-white border border-emerald-200 rounded p-2.5 mb-2">
            <p className="text-[11px] text-slate-600 italic">{aiEnabled.query}</p>
          </div>
          <p className="text-[11px] text-slate-500 mb-1">Scoring: {aiEnabled.scoring.join(' | ')}</p>
          <p className="text-[11px] text-slate-500">Quality: <span className="font-bold text-emerald-700">{aiEnabled.quality}</span></p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden mb-3">
        <div className="bg-slate-800 px-4 py-2">
          <p className="text-xs font-bold text-white uppercase tracking-wider">Top 3 Recommendations</p>
        </div>
        <div className="grid grid-cols-3 gap-0 divide-x divide-slate-200">
          {aiEnabled.topResults.map((fund: any) => (
            <div key={fund.rank} className="p-3">
              <div className="flex items-center gap-1.5 mb-1.5">
                <span className="text-xs font-bold text-amber-600">#{fund.rank}</span>
      ...
      <div className="bg-gradient-to-r from-teal-600 to-teal-700 rounded-lg p-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-bold text-white">Time Saved: {timeSaved}</span>
        </div>
        <span className="text-xs text-teal-200">Total Process: {totalProcess}</span>
      </div>
    </DeepDiveSlideLayout>
  );
};
