    ...
    <DeepDiveSlideLayout slideNumber={slideNumber} totalSlides={totalSlides} sectionLabel="AI Nudges" sectionColor="bg-blue-600">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {phases.map((phase: any, i: number) => (
          <div key={i} className="bg-slate-50 border border-slate-200 rounded-lg p-2.5">
            <p className="text-[10px] font-bold text-slate-800 mb-0.5">{phase.phase}</p>
            <p className="text-[8px] text-slate-500 mb-1.5">{phase.period}</p>
            
            {phase.variants && (
              <div className="space-y-1 mb-1.5">
                {phase.variants.map((v: any, j: number) => (
                  <div key={j} className={`text-[8px] p-1 rounded ${v.name === phase.winner?.split(' ')[0] + ' ' + phase.winner?.split(' ')[1] ? 'bg-emerald-50 border border-emerald-200' : 'bg-white border border-slate-100'}`}>
                ...
                {phase.winner && <p className="text-[8px] font-bold text-emerald-600">Winner: {phase.winner}</p>}
              </div>
            )}
            ...
            {phase.improvements && (
              <div className="space-y-0.5 mb-1.5">
                {phase.improvements.map((imp: string, j: number) => (
                  <p key={j} className="text-[8px] text-slate-600">✓ {imp}</p>
                ))}
              </div>
            )}
            ...
            {phase.patterns && (
              <div className="space-y-0.5 mb-1.5">
                {phase.patterns.map((p: string, j: number) => (
                  <p key={j} className="text-[8px] text-slate-600">💡 {p}</p>
                ))}
              </div>
            )}
            ...
            {phase.overall && <p className="text-[9px] font-bold text-emerald-700 mt-1.5">{phase.overall}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-r from-slate-800 to-slate-700 rounded-lg p-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <p className="text-[9px] text-slate-400 mb-1">Baseline (Week 1):</p>
            <p className="text-[8px] text-slate-300 italic">"{sipExample.baseline.message}"</p>
            <p className="text-[8px] text-red-400 mt-1">Open: {sipExample.baseline.openRate} | Action: {sipExample.baseline.actionRate}</p>
          </div>
          <div>
            <p className="text-[9px] text-emerald-400 mb-1">Optimized (Week 8):</p>
            <p className="text-[8px] text-slate-300 italic">"{sipExample.optimized.message}"</p>
            <p className="text-[8px] text-emerald-400 mt-1">Open: {sipExample.optimized.openRate} | Action: {sipExample.optimized.actionRate}</p>
            <div className="flex gap-2 mt-1">
              {sipExample.improvements.map((imp: string, i: number) => (
                <span key={i} className="text-[8px] bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 rounded">{imp}</span>
              ))}
            </div>
          </div>
        </div>
