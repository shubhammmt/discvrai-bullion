  ...
  return (
    <DeepDiveSlideLayout slideNumber={slideNumber} totalSlides={totalSlides} sectionLabel="Personalization" sectionColor="bg-purple-600">
      <div className="mb-2">
        ...
        <p className="text-base text-slate-500">{slide.subtitle} — <span className="font-semibold">{product}</span></p>
      </div>

      {/* Factors */}
      <div className="space-y-2 mb-3">
        {factors.map((factor: any, i: number) => (
          <div key={i} className="bg-slate-50 border border-slate-200 rounded-lg p-2.5 flex items-center gap-3">
            <div className="w-32 shrink-0">
              <p className="text-xs font-bold text-slate-700">{factor.name}</p>
              <p className="text-[11px] text-slate-500">Weight: {factor.weight}</p>
            </div>
            <div className="flex-1">
              <div className="w-full bg-slate-200 rounded-full h-3">
                <div className={`${barColors[i]} h-3 rounded-full`} style={{ width: `${factor.score}%` }} />
              </div>
            </div>
            <div className="w-16 text-right shrink-0">
              <span className="text-sm font-mono font-bold text-slate-700">{factor.score}</span>
              <span className="text-[11px] text-slate-500">/100</span>
            </div>
            <div className="w-16 text-right shrink-0">
              <span className="text-xs font-bold text-emerald-600">{factor.weighted.toFixed(1)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-slate-800 rounded-lg p-3 mb-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-base font-bold text-amber-400">{rank}</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="col-span-2 bg-amber-50 border border-amber-200 rounded-lg p-3">
          <p className="text-[11px] text-slate-700 italic leading-snug">{message}</p>
        </div>
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex items-center justify-center">
          <div className="text-center">
            <p className="text-xs text-slate-500">Conversion Rate</p>
            <p className="text-xl font-bold text-emerald-700">{conversionRate}</p>
          </div>
        </div>
      </div>
    </DeepDiveSlideLayout>
