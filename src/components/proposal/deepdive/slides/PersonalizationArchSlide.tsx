          ...
          return (
            <React.Fragment key={i}>
              <div className={`${colors.bg} border ${colors.border} rounded-lg p-2.5`}>
                <p className={`text-[10px] font-bold ${colors.text} uppercase tracking-wider mb-1`}>{layer.name}</p>
                {isArrayItems ? (
                  <div className="flex flex-wrap gap-1.5">
                    {layer.items.map((item: string, j: number) => (
                      <span key={j} className="text-[9px] bg-white border border-slate-200 rounded px-2 py-0.5 text-slate-700">{item}</span>
                    ))}
                  </div>
                ) : (
                  <div className="flex gap-2">
                    {layer.items.map((group: any, j: number) => (
                      <div key={j} className="bg-white border border-slate-200 rounded px-2 py-1.5 flex-1">
                        <p className="text-[9px] font-bold text-slate-700 mb-0.5">{group.label}</p>
                        {group.items.map((item: string, k: number) => (
                          <p key={k} className="text-[8px] text-slate-500 leading-tight">• {item}</p>
                        ))}
                      </div>
                    ))}
                  </div>
