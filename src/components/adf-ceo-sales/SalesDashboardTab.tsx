...
const QtrTable = ({ title, data, nameKey, showStatus, icon }: { title: string; data: any[]; nameKey: string; showStatus?: boolean; icon?: React.ReactNode }) => (
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-gray-200">
            ...
            {showStatus && <TableHead className={`${th} min-w-[120px]`}>Status</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {data.map((r: any, i: number) => {
            const isTotalRow = r[nameKey] === 'TOTAL';
            return (
              <TableRow key={r[nameKey]} className={`border-gray-100 transition-colors hover:bg-indigo-50/40 ${isTotalRow ? 'font-bold bg-indigo-50/60' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                ...
                {showStatus && (
                  <TableCell className={td}>
                    {r.status ? <span className={`text-[11px] px-2.5 py-1 rounded-full font-semibold whitespace-nowrap ${signalColor(r.status)}`}>{r.status}</span> : '—'}
                  </TableCell>
                )}
              </TableRow>
            );
...
export const SalesDashboardTab: React.FC = () => (
  <div className="space-y-6">
    ...
    <QtrTable title="Zone-wise Performance | Q1–Q3, 9M FY26 vs FY25" data={salesDashboard.byZone} nameKey="region" showStatus icon={<Globe className="w-4 h-4 text-sky-500" />} />
  </div>
);
