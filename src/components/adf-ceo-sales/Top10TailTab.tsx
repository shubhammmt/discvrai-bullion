...
const TopTable = ({ title, data, nameKey, showCum, icon }: { title: string; data: any[]; nameKey: string; showCum?: boolean; icon?: React.ReactNode }) => (
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-gray-200">
            ...
            {showCum && <TableHead className={`${th} text-right min-w-[70px]`}>Cum%</TableHead>}
        ...
        <TableBody>
          {data.map((r: any, i: number) => (
            <TableRow key={r[nameKey]} className={`border-gray-100 transition-colors hover:bg-indigo-50/40 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
              ...
              {showCum && <TableCell className={`${td} text-right tabular-nums text-gray-600`}>{r.cumPct}%</TableCell>}
              <TableCell className={td}>{r.status ? <span className={`text-[11px] px-2.5 py-1 rounded-full font-semibold whitespace-nowrap ${signalColor(r.status)}`}>{r.status}</span> : '—'}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
