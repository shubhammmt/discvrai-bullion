...
const GrowthTable = ({ title, data, nameKey, showQuarters, icon }: { title: string; data: any[]; nameKey: string; showQuarters?: boolean; icon?: React.ReactNode }) => (
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-gray-200">
            <TableHead className={`${th} min-w-[160px]`}>{nameKey === 'category' ? 'Category' : nameKey === 'brand' ? 'Brand' : 'Region'}</TableHead>
            ...
            {showQuarters && <TableHead className={`${th} text-right min-w-[80px]`}>Q1 Gr%</TableHead>}
            {showQuarters && <TableHead className={`${th} text-right min-w-[80px]`}>Q2 Gr%</TableHead>}
...
export const GrowthAnalysisTab: React.FC = () => (
  <div className="space-y-6">
    <GrowthTable title="Category Growth & Degrowth Analysis" data={growthAnalysis.categories} nameKey="category" showQuarters icon={<TrendingUp className="w-4 h-4 text-emerald-500" />} />
    <GrowthTable title="Brand Growth & Degrowth Analysis" data={growthAnalysis.brands} nameKey="brand" showQuarters icon={<TrendingUp className="w-4 h-4 text-purple-500" />} />
    <GrowthTable title="Region Growth & Degrowth Analysis" data={growthAnalysis.regions} nameKey="region" showQuarters icon={<Globe className="w-4 h-4 text-sky-500" />} />
  </div>
);
