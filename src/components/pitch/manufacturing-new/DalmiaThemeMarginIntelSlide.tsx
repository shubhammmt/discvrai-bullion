...
import { TrendingUp as TrendingUpIcon, Server, Landmark, FileText, Brain, LayoutDashboard, MessageSquare, AlertCircle, Sparkles } from 'lucide-react';

interface DalmiaThemeMarginIntelSlideProps {
  slide: ManufacturingNewSlide;
  slideNumber: number;
  totalSlides: number;
}

export const DalmiaThemeMarginIntelSlide: React.FC<DalmiaThemeMarginIntelSlideProps> = ({
  slideNumber,
  totalSlides
}) => {
  const problems = [
  ...
  const sources = [
    { label: 'Price Feeds', icon: TrendingUpIcon },
      ...
      <div className="flex flex-col h-full">
        ...
        >
          <span className="inline-block px-3 py-1 text-xs font-bold uppercase tracking-widest text-amber-600 bg-amber-50 rounded-full border border-amber-200">
            Theme 5 — Financial Intelligence
          </span>
        </motion.div>
        ...
        >
          AI Margin Intelligence & Financial Automation
        </motion.h1>
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3">
            ...
            <div className="flex flex-col items-center">
              <div className="w-44 h-14 rounded-xl bg-gradient-to-r from-purple-500 to-purple-600 flex items-center justify-center shadow-md">
                <Brain className="w-5 h-5 text-white mr-2" />
                <div className="text-center">
                  <span className="text-sm font-bold text-white block">AI ENGINE</span>
                  <span className="text-xs text-purple-200">Margin Simulation</span>
                </div>
              </div>
            </div>
            ...
            {/* Output: P&L Dashboard + Chatbot */}
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-center">
                <div className="w-16 h-14 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 flex items-center justify-center shadow-md">
                  <LayoutDashboard className="w-6 h-6 text-white" />
                </div>
                <span className="text-xs text-slate-600 mt-1 font-medium">P&L Dashboard</span>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-16 h-14 rounded-xl bg-gradient-to-r from-teal-500 to-teal-600 flex items-center justify-center shadow-md">
                  <MessageSquare className="w-6 h-6 text-white" />
                </div>
                <span className="text-xs text-slate-600 mt-1 font-medium">AI Chat Bot</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        >
          <span className="text-xs text-slate-500">
            Case Studies: <span className="font-semibold text-slate-700">Commodity Costing AI</span> • <span className="font-semibold text-slate-700">Financial Reconciliation Automation</span>
          </span>
        </motion.div>
      </div>
    </MfgNewSlideLayout>
