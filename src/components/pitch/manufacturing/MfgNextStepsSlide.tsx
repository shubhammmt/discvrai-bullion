...
interface MfgNextStepsSlideProps {
  slide: {
    headline: string;
    subheadline: string;
    steps: Array<{
...
export const MfgNextStepsSlide: React.FC<MfgNextStepsSlideProps> = ({ slide, slideNumber, totalSlides }) => {
  return (
                ...
                <div className="ml-11 space-y-1">
                  {step.details.map((detail, detIndex) => (
                    <div key={detIndex} className="flex items-start gap-2">
                      <ArrowRight className="w-3 h-3 text-amber-500 flex-shrink-0 mt-1" />
          ...
          <div className="col-span-5 space-y-3">
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-emerald-700">PoC Success Criteria</h3>
              </div>
              <div className="space-y-2">
                {slide.pocCriteria.map((criteria, index) => (
                  <div key={index} className="flex items-start gap-2">
                    <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" />
                    <p className="text-sm text-emerald-700">{criteria}</p>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-white">Post-PoC Path</h3>
              </div>
              <div className="space-y-2">
