...
import { SIP_STATEMENT_COLORS } from '@/config/sipBrandConfig';
...
const STATEMENT_OPTIONS: StatementOption[] = [
  { id: 'account', title: 'Account Statement', description: 'Complete portfolio statement with all holdings, NAV, and units', icon: FileText, formats: ['PDF', 'Excel'] },
  { id: 'capital-gains', title: 'Capital Gains Report', description: 'STCG & LTCG details for tax filing (ITR)', icon: IndianRupee, formats: ['PDF', 'Excel'] },
  { id: 'transaction', title: 'Transaction Summary', description: 'All buy, sell, SIP, and switch transactions', icon: BarChart3, formats: ['PDF', 'Excel', 'CSV'] },
  { id: 'tax-saving', title: 'Tax Saving Certificate', description: '80C investment proof for ELSS funds', icon: Shield, formats: ['PDF'] },
];
...
export function StatementsTab() {
  ...
  const handleDownload = (title: string, fmt: string) => {
    toast.success(`Downloading ${title}`, { description: `${fmt} file will be ready shortly` });
  };
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            <FileText className="w-4 h-4 text-sip-brand" />
            Generate Statement
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-0 space-y-3">
          {STATEMENT_OPTIONS.map(opt => {
            const Icon = opt.icon;
            const colorToken = SIP_STATEMENT_COLORS[opt.id] || 'sip-action-info';
            return (
              <div key={opt.id} className="rounded-lg border border-border p-3 space-y-2">
                <div className="flex items-start gap-3">
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                    style={{ backgroundColor: `hsl(var(--${colorToken}) / 0.15)`, color: `hsl(var(--${colorToken}))` }}>
