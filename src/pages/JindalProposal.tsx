...
import { ProposalSlideRenderer } from '@/components/proposal/ProposalSlideRenderer';
import { jindalProposalSlides } from '@/data/jindalProposalSlides';
import { ChevronLeft, ChevronRight, Bot } from 'lucide-react';

const JindalProposal = () => {
  const navigate = useNavigate();
  const [currentSlide, setCurrentSlide] = useState(0);

  const nextSlide = () => {
    setCurrentSlide((prev) => Math.min(prev + 1, jindalProposalSlides.length - 1));
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        ...
        >
          <ProposalSlideRenderer slide={jindalProposalSlides[currentSlide]} />
        </div>
      </div>
      ...
      <div className="bg-white border-t border-slate-200 px-8 py-4">
        <div className="max-w-[1280px] mx-auto flex items-center justify-between">
          ...
          {/* Slide indicators */}
          <div className="flex items-center gap-2">
            {jindalProposalSlides.map((_, index) => (
              <button
                key={index}
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-slate-500">
              {currentSlide + 1} / {jindalProposalSlides.length}
            </span>
            <div className="flex gap-2">
              ...
              <button
                onClick={nextSlide}
                disabled={currentSlide === jindalProposalSlides.length - 1}
