          ...
          <div className="space-y-2 max-h-[640px] overflow-auto pr-1">
            {incidents.map(inc => (
              <button key={inc.id} onClick={() => setSelId(inc.id)}
                className={`w-full text-left rounded-lg border p-3 transition ${selId === inc.id ? 'border-cyan-400 bg-cyan-50' : 'border-slate-200 bg-slate-50 hover:bg-slate-100'}`}>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-cyan-700 font-mono">{inc.id}</span>
                  <SevPill sev={inc.sev} />
                </div>
                <div className="text-sm text-slate-900 mt-1">{inc.title}</div>
                <div className="text-[11px] text-slate-500 mt-1.5 flex flex-wrap gap-x-3">
                  <span>📍 {inc.station}</span>
                  <span>👤 {inc.owner}</span>
                  <span>⏱ {inc.age}</span>
                  <span className="text-cyan-700">{inc.status}</span>
                </div>
              </button>
            ))}
          </div>
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-cyan-700 font-mono">{sel.id}</span>
                  <SevPill sev={sel.sev} />
                  <span className="text-[11px] text-slate-500">Source zone: {sel.source}</span>
                </div>
                <h2 className="text-xl text-slate-900 mt-2">{sel.title}</h2>
                <div className="text-[11px] text-slate-500 mt-1">Affected: {sel.station} · Owner: {sel.owner} · Status: {sel.status}</div>
              </div>
              <div className={`rounded-xl border p-3 text-right ${sel.cert.reported ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-orange-500/30 bg-orange-500/10'}`}>
                <div className="text-[10px] uppercase tracking-wider text-slate-600 flex items-center justify-end gap-1"><Clock className="w-3 h-3" /> CERT-In · 6h reporting</div>
                <div className={`text-2xl font-semibold mt-1 ${sel.cert.reported ? 'text-emerald-700' : 'text-orange-700'}`}>{sel.cert.reported ? 'Filed' : sel.cert.reportBy}</div>
                <div className="text-[11px] text-slate-500">{sel.cert.reported ? 'On record' : 'Time remaining'}</div>
              </div>
            </div>
          </Card>
            ...
            <ol className="relative border-l border-slate-200 ml-2 space-y-4">
              {incidentTimeline.map((e, i) => (
                <li key={i} className="ml-4">
                  ...
                  <div className="text-[11px] text-cyan-700 font-mono">{e.t} · {e.ev}</div>
                  <div className="text-sm text-slate-800">{e.detail}</div>
                </li>
              ))}
            </ol>
          </Card>
          ...
          <Card title="Response playbook · recommended actions">
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button key={t} className="text-left rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100 p-3 flex items-start gap-3">
                  <Icon className="w-4 h-4 text-cyan-700 mt-0.5" />
                  <div>
