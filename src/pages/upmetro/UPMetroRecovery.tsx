            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {recoverySystems.map(s => (
                  <tr key={s.sys} className="border-t border-white/5">
                    <td className="px-3 py-2.5 text-slate-100">{s.sys}</td>
                    <td className="px-3 py-2.5">{s.immutable ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <AlertTriangle className="w-4 h-4 text-red-400" />}</td>
                    <td className="px-3 py-2.5 text-slate-300">{s.lastTest}</td>
                    <td className="px-3 py-2.5 text-slate-300">{s.rto}</td>
                    <td className="px-3 py-2.5 text-slate-400">{s.rtoTarget}</td>
                    <td className="px-3 py-2.5 text-slate-300">{s.rpo}</td>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card title="Drill planner" right={<button className="text-xs px-2 py-1 rounded bg-cyan-400 text-[#06142A] font-semibold">+ Schedule drill</button>}>
            <div className="space-y-2">
              {drills.map(d => (
                <div key={d.name} className="rounded bg-white/5 border border-white/10 p-3 flex items-center gap-3">
                  <Calendar className="w-4 h-4 text-cyan-300" />
                  <div className="flex-1">
                    <div className="text-sm">{d.name}</div>
                    <div className="text-[11px] text-slate-400">{d.date} · {d.city} · {d.participants} participants</div>
                  </div>
                  <span className={`text-[10px] px-2 py-0.5 rounded border ${d.status === 'Scheduled' ? 'bg-cyan-500/15 text-cyan-300 border-cyan-500/30' : 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30'}`}>{d.status}</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card title="After-action findings · closure tracker">
            <div className="space-y-2">
              {drills.filter(d => d.status === 'Closed').map(d => (
                <div key={d.name} className="rounded bg-white/5 border border-white/10 p-3">
                  <div className="text-sm">{d.name}</div>
                  <div className="text-[11px] text-slate-400 mt-0.5">{d.date} · {d.findings} findings · {d.openFindings} open</div>
              ...
              <div className="rounded border border-dashed border-white/10 p-3 text-xs text-slate-400">
