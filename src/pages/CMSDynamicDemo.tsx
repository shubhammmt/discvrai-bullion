...
import IntroScreen from '@/components/cms-dynamic/IntroScreen';
import ReconciliationScreen from '@/components/cms-dynamic/ReconciliationScreen';
import IntegrationScreen from '@/components/cms-dynamic/IntegrationScreen';
import RouteDispatchScreen from '@/components/cms-dynamic/RouteDispatchScreen';
import SummaryScreen from '@/components/cms-dynamic/SummaryScreen';

const STEPS = [
  { label: 'Financial Reconciliation', num: 1 },
  { label: 'Integration & Testing', num: 2 },
  { label: 'Route & Dispatch', num: 3 },
];

const CMSDynamicDemo = () => {
    ...
    <div className="min-h-screen" style={{ background: '#0A0F1E' }}>
            ...
            {STEPS.map(s => {
              ...
              return (
                <span key={s.num} className={`text-xs font-medium flex items-center gap-1.5 ${isActive ? 'text-white' : isCompleted ? 'text-emerald-400' : 'text-gray-600'}`}>
                  ...
                  {s.label}
                  {isActive && <span className="block h-0.5 w-full rounded-full mt-0.5 absolute bottom-0" style={{ background: '#6366F1' }} />}
                </span>
              );
            })}
          </div>
          <div className="flex items-center gap-2 text-gray-600 text-xs">
            <span>Demo · Agentic Operations Platform</span>
      ...
      <div className={showChrome ? 'pt-14 pb-16' : ''}>
        <AnimatePresence mode="wait">
          {screen === 0 && <IntroScreen key="intro" onStart={() => setScreen(1)} />}
          {screen === 1 && <ReconciliationScreen key="recon" />}
          {screen === 2 && <IntegrationScreen key="integration" />}
          {screen === 3 && <RouteDispatchScreen key="dispatch" />}
          {screen === 4 && <SummaryScreen key="summary" onRestart={() => setScreen(0)} />}
        </AnimatePresence>
      </div>
