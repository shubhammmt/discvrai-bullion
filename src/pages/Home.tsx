import React from 'react';
import { useNavigate } from 'react-router-dom';
import Header from '@/components/Header';
import GlobalFooter from '@/components/GlobalFooter';
import { QuickPollWidget } from '@/components/engagement/QuickPollWidget';
import { TrendingPollResults } from '@/components/engagement/TrendingPollResults';
import NewsCarousel from '@/components/mutualfunds/NewsCarousel';
import { ProductFeatureCard } from '@/components/news/ProductFeatureCard';
import FundThemes from '@/components/mutualfunds/FundThemes';
import HeadlineScroller from '@/components/HeadlineScroller';
import { Button } from '@/components/ui/button';
import { Card } from '@/components/ui/card';
import { Progress } from '@/components/ui/progress';
import { MUTUAL_FUND_FEATURES } from '@/data/productFeatures';
import { mockPolls } from '@/data/mockEngagementData';
import { TrendingUp, Users } from 'lucide-react';
import { SEOHead } from '@/components/seo/SEOHead';

const Home = () => {
  const navigate = useNavigate();

  // Get top 3 active polls
  const topPolls = mockPolls
    .sort((a, b) => b.totalVotes - a.totalVotes)
    .slice(0, 3);

  const handleExploreTheme = (themeId: string) => {
    navigate(`/mutual-fund-research?theme=${themeId}`);
  };

  return (
    <div className="min-h-screen bg-background">
      <SEOHead 
        title="DiscvrAI - Discover Smart Investments with AI"
        description="Your intelligent companion for mutual funds, stocks, and financial insights. Get personalized recommendations, real-time news, and community-driven polls."
        canonical="https://www.discvr.ai/home"
      />
      
      <Header />
      
      {/* Hero Intro */}
      <section className="container mx-auto px-4 py-12 text-center max-w-4xl">
        <h1 className="text-4xl md:text-5xl font-bold mb-4">
          Your AI-Powered Financial Companion
        </h1>
        <p className="text-lg md:text-xl text-muted-foreground">
          Discover smarter investments with <span className="text-blue-500 font-semibold">AI research</span>, <span className="text-emerald-500 font-semibold">real-time market insights</span>, and a community of <span className="text-purple-500 font-semibold">informed investors</span>. 
          From mutual funds to stocks, get personalized recommendations that match your goals.
        </p>
      </section>
      
      <main className="container mx-auto px-4 py-8 space-y-12">
        {/* Polls Section */}
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-bold flex items-center gap-2">
                <Users className="h-8 w-8 text-primary" />
                Top Community Polls
              </h2>
              <p className="text-muted-foreground mt-2">
                See what the community thinks and share your opinion
              </p>
            </div>
            <Button 
              className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold shadow-lg"
              onClick={() => navigate('/polls')}
            >
              View All Polls
            </Button>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {/* Interactive Poll - Dark Blue */}
            <div className="space-y-4">
              {topPolls[0] && (
                <Card className="bg-blue-950 border-blue-800 text-white">
                  <QuickPollWidget poll={topPolls[0]} />
                </Card>
              )}
            </div>

            {/* Next 2 Polls with Options */}
            {topPolls.slice(1, 3).map((poll) => (
              <Card key={poll.id} className="p-6 border-l-4 border-l-primary/50">
                <h3 className="font-semibold text-lg mb-4">{poll.question}</h3>
                <div className="space-y-3">
                  {poll.options.map((option) => (
                    <div key={option.id} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium">{option.text}</span>
                        <span className="text-primary font-bold">{option.percentage}%</span>
                      </div>
                      <Progress value={option.percentage} className="h-2" />
                    </div>
                  ))}
                </div>
                <div className="flex items-center gap-1 text-xs text-muted-foreground mt-4">
                  <Users className="w-3 h-3" />
                  {poll.totalVotes.toLocaleString()} votes
                </div>
              </Card>
            ))}
          </div>
        </section>

        {/* News Section with Headline Scroller */}
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-bold text-left">Latest Market News</h2>
              <p className="text-muted-foreground mt-2 text-left">
                Stay updated with real-time financial news and insights
              </p>
            </div>
            <Button 
              variant="outline" 
              onClick={() => navigate('/news')}
            >
              View All News
            </Button>
          </div>
          
          <HeadlineScroller />
          
          <NewsCarousel />
        </section>

        {/* Deep Research Section */}
        <section className="space-y-6">
          <div className="max-w-3xl">
            <h2 className="text-3xl font-bold text-left">Deep Research at your fingertips</h2>
            <p className="text-muted-foreground mt-2 text-left">
              AI-powered tools to help you make smarter investment decisions
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {MUTUAL_FUND_FEATURES.map((feature) => (
              <ProductFeatureCard key={feature.id} feature={feature} />
            ))}
          </div>
        </section>

        {/* Mutual Funds Section */}
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h2 className="text-3xl font-bold text-left">Popular Investment Themes</h2>
              <p className="text-muted-foreground mt-2 text-left">
                Curated themes and trending funds personalized for you
              </p>
            </div>
            <Button 
              variant="outline" 
              onClick={() => navigate('/mutual-fund-research')}
              className="flex-shrink-0"
            >
              View All Funds
            </Button>
          </div>

          <FundThemes onExploreTheme={handleExploreTheme} />
        </section>
      </main>

      <GlobalFooter />
    </div>
  );
};

export default Home;
